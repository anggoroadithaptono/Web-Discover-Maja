...
<div class="filter-container">
  <form method="GET" action="{{ route('homepromositur.index') }}">
  ...
  <div class="promo-buttons">
    <a href="{{ route('promosi-tur.lihatPembelian') }}" class="button orange-btn">Lihat Pembelian</a>
    <a href="{{ route('promosi-tur.create') }}" class="button orange-btn">Tambah Promosi</a>
  </div>
</div>

<!-- Formulir Pencarian -->
<form action="{{ route('homepromositur.index') }}" method="GET" class="search-form" style="text-align:center; margin:20px 0;">
    ...
    @if ($promosi_tur->isNotEmpty())
      @foreach ($promosi_tur as $promo)
        <div class="promo-card">
          ...
          <a href="{{ route('promosi-tur.detail', $promo->id) }}" class="delete-button">Lihat Detail</a>

          <!-- Form hapus promosi -->
          <form action="{{ route('hapus.promosi', $promo->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus promosi ini?')">
              @csrf
              @method('DELETE')
